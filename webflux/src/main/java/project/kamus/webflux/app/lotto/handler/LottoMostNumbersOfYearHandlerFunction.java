package project.kamus.webflux.app.lotto.handler;
...
import project.kamus.webflux.app.lotto.dto.LottoHistoryDto;
import project.kamus.webflux.app.lotto.dto.LottoMostNumbersOfYearDto;
import project.kamus.webflux.app.lotto.dto.LottoNumberCountDto;
import project.kamus.webflux.app.lotto.util.LottoHistoryUtil;
import project.kamus.webflux.config.ResponseWrapper;
import reactor.core.publisher.Mono;

@Slf4j
@Component
public class LottoMostNumbersOfYearHandlerFunction implements HandlerFunction<ServerResponse> {
  ...
  public Mono<ServerResponse> handle(@NonNull ServerRequest request) {
    ...
    List<LottoMostNumbersOfYearDto> result = new ArrayList<>();
    ...
    IntStream.rangeClosed(2002, Year.now().getValue())
        .forEach(year -> {
          ...
          result.add(LottoMostNumbersOfYearDto.builder().year(year).numbers(mostNumbers).build());
        });
